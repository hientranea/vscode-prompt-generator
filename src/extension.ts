...
export function activate(context: vscode.ExtensionContext): void {
  const disposable = vscode.commands.registerCommand("prompt-generator.generate", async () => {
    // Add status bar item
    ...
    const templatesAndRules = config.get("templatesAndRules") as {
      templates: Template[];
    };
    ...
    const wrappedContents = await Promise.all(
    ...
    vscode.commands.registerCommand("fileConcatenator.editTemplates", () => {
      ...
      vscode.workspace.applyEdit(workspaceEdit).then(() => {
        vscode.commands.executeCommand(
          "vscode.openWith",
          filePath,
          "fileConcatenator.templateEditor"
        );
      });
    })
  );
